...
third line: sampling parameter for configurations, i.e. configurations are considered every nskip (> nskip)
(if nskip = 1, all configurations are considered)
fourth line: time between two considered configurations (> dtime)
fifth line: number of species for which the calculation will be done (> nspecies) !MAX 1000!
...
int **normI, **normC, nconfigs, nspecies, nionstot, ncorr, nskip;
double zmin, zmax;
...
void read()
{
 ...
 scanf("%d",&nskip);		/* number of skips between considered configurations */
 scanf("%lf",&dtime);		/* time between two considered configurations in ps */
	     ...
	     if(itype!=0)
	       {
	        ...
	        if((z<=zmax)&&(z>=zmin)) 
	          {
	           storeI[j][iii]=1; storeC[j][iii]=1;
	          }
	        if((z>zmax)||(z<zmin)) 
	          {
	           storeI[j][iii]=0; storeC[j][iii]=0;
	          }
	        dstoreI=storeI[j][iii];
                dstoreC=storeC[j][iii];
	        for(k=ncorr;k>=iii+1;k--) 
	           {
	            rvacfI=dstoreI*storeI[j][k]; cfI[iii-k+ncorr][itype]+=rvacfI; 
                    normI[iii-k+ncorr][itype]++;
	            prodstoreC=storeC[j][k];
	            n=k+1;
		    ...
		    n=1;
		    while((n<iii)&&(prodstoreC==1))
		         {
		   	  prodstoreC*=storeC[j][n];
			  n++;
		         }
		    rvacfC=dstoreC*prodstoreC; cfC[iii-k+ncorr][itype]+=rvacfC; 
                    normC[iii-k+ncorr][itype]++;
		   }
	        for(k=iii;k>=1;k--) 
	           {
	            rvacfI=dstoreI*storeI[j][k]; cfI[iii-k][itype]+=rvacfI; 
		    normI[iii-k][itype]++;
		    prodstoreC=storeC[j][k];
                    for(n=k+1;n<iii;n++) 
		       {
		        if(n>iii) printf("Problem\n"); 
		        prodstoreC*=storeC[j][n];
		       }
		    rvacfC=dstoreC*prodstoreC; cfC[iii-k][itype]+=rvacfC; 
 		    normC[iii-k][itype]++;
		   }		    
	        }
             } /* end loop nionstot */
